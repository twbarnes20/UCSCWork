...
{
  std::vector<Attrs> attrs = {ATTR_array, ATTR_int, ATTR_string, ATTR_struct};
    ...
    { 
      ...
      if (tree->children.at(0)->symbol == TOK_INT)
      ...
      else if (tree->children.at(0)->symbol == TOK_TYPEID)
        {
          ...
          tree->children.back()->type = (string *)tree->children.at(0)->lexinfo;
          tree->children.back()->attribs.set(ATTR_struct);
        }
    }
...
dfs_return_value *postorder_dfs_traversal(astree* tree)
{
  ...
  if (tree->symbol == TOK_ROOT)
  ...
  else if (tree->symbol == TOK_VARDECL) 
    {
      ...
      if (!is_compat(declid->attribs, right_child->attribs,
                      declid->type, right_child->type))
        {
      ...
      if(right_child == NULL)
	{
      ...
      if (!primitive(left_child->attribs))
        { 
          fprintf(stderr, "left_child operand of %s is not primitive\n", op);
        }

      if (!primitive(right_child->attribs))
        { 
          fprintf(stderr, "right_child operand of %s is not primitive\n", op);
        }
      ...
      if(right_child == NULL)
	{
      ...
      if ((left_child->attribs[ATTR_int] && right_child->attribs[ATTR_int])
        ...
        {
          if (tree->symbol == '+')
            { fprintf(stderr, "Not valid: binary +\n"); }
          else if (tree->symbol == '-')
            { fprintf(stderr, "Not valid: binary -\n"); }
          else if (tree->symbol == '*')
	...
	{
	  if (!is_compat(temp_symb->parameters->at(k-1)->attribs, tree->children.at(k)->attribs,
			  temp_symb->parameters->at(k-1)->type_id, tree->children.at(k)->type))
	    {
	      //fprintf(stderr, "Invalid function arguments %s\n", tree->children.at(0)->lexinfo->c_str());
	    }
	} 
