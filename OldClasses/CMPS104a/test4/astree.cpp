...
void astree::print (FILE* outfile, astree* tree, int depth) {
   ...
   fprintf (outfile, "%s \"%s\" (%zd.%zd.%zd) {%zd} ",
	    ...
	    tree->lloc.blocknr);
   print_attributes(outfile, tree->attribs, tree->type, NULL);
   fprintf(outfile," \n");
...
static void print_attribute(FILE *out, string attr, string *extra)
{
      ...
      if (attribs.test(i))
        {
          ...
          print_attribute(out, attr_str_map[attri], extra);
        }
    }
}
