...
identdecl : basetype TOK_ARRAY TOK_IDENT
                                         ...
                                         (TOK_DECLID);  
                                         $$ = $1 -> adopt($2);}
          | basetype TOK_IDENT           {$2 -> change_sym 
                                         (TOK_DECLID);
                                         $$ = $1 -> adopt($2);}  
          ;
...
allocator : TOK_NEW TOK_IDENT '(' ')'   {$2 -> change_sym(TOK_TYPEID);
          ...
          | TOK_NEW TOK_STRING '(' expr ')'
                                        {$$ = $1 -> 
                                        adopt_sym($4, TOK_NEWSTRING);
                                        destroy($3, $5);}
          | TOK_NEW basetype '[' expr ']'
                                        {$$ = $1 -> 
                                        adopt_sym($4, TOK_NEWARRAY);
                                        destroy($3, $5);}
          ;

call      : TOK_IDENT '(' ')'           {$$ = $2 -> 
                                        adopt_sym($1, TOK_CALL);
                                        destroy($3);}
          | TOK_IDENT call_args ')'     {$$ = $2 -> 
                                        adopt_sym($1, TOK_CALL);
                                        destroy($3);} 
          ;

call_args :  '(' expr                   {$$ = $1 -> adopt($2);}
