    ...
    let rec mul'' max list1' list2' =  
      let relate = num_comp max list1' 
        in if relate = Less then 
          ...
          let result = mul'' max (remove_zero(add' list1' list1' 0)) 
                                 (remove_zero(add' list2' list2' 0))
          in let relate2 = num_comp result.remainder list1'
          in if relate2 <> Less then
            {remainder = remove_zero (sub' result.remainder list1' 0) 
                     ; sum = remove_zero (add' result.sum list2' 0) }
          else
            result 
    ...
    let rec div'' max list1' list2' = 
      let relate = num_comp max list2'
        in if relate = Less then
          ...
          let result = div'' max (remove_zero(add' list1' list1' 0)) 
                                 (remove_zero(add' list2' list2' 0))
          in let relate2 = num_comp result.remainder list2'
          in if relate2 <> Less then
            {remainder = remove_zero (sub' result.remainder list2' 0) 
                     ; sum = remove_zero (add' result.sum list1' 0) }
          else
            result
  ...
  let pow' x n =
    let rec pow'' y x n =
      if n = []
        ...
        pow'' (mul' x y) (mul' x x) (fst(div' 
            (remove_zero (sub' n [1] 0)) [2]))   
    in pow'' [1] x n 
